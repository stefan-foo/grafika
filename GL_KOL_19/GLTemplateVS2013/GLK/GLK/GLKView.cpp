...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case VK_DOWN:
		m_glRenderer.m_a -= 5;
		if (m_glRenderer.m_a <= 1) {
			m_glRenderer.m_a = 1;
		}
		break;
	case 'Q':
		m_glRenderer.m_dist += 2;
		if (m_glRenderer.m_dist >= 50) {
			m_glRenderer.m_dist = 50;
		}
		break;
	case 'W':
		m_glRenderer.m_dist -= 2;
		if (m_glRenderer.m_dist <= 8) {
			m_glRenderer.m_dist = 8;
		}
		break;
	case '1':
		m_glRenderer.kabina12 += 5;
		break;
	case '2':
		m_glRenderer.kabina12 -= 5;
		break;
	case '3':
		m_glRenderer.ruka34 += 5;
		break;
	case '4':
		m_glRenderer.ruka34 -= 5;
		break;
	case '5':
		m_glRenderer.drukaruka56 += 5;
		break;
	case '6':
		m_glRenderer.drukaruka56 -= 5;
		break;
	case '7':
		m_glRenderer.viljuska78 += 5;
		break;
	case '8':
		m_glRenderer.viljuska78 -= 5;
		break;
