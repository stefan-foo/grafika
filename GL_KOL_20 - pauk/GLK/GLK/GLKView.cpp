...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar) {
	case VK_UP:
		m_glRenderer.m_alpha += 5;
		break;
	case VK_DOWN:
		m_glRenderer.m_alpha -= 5;
		break;
	case VK_RIGHT:
		m_glRenderer.m_beta -= 5;
		break;
	case VK_LEFT:
		m_glRenderer.m_beta += 5;
		break;
	case VK_ADD:
		m_glRenderer.m_dist -= 5;
		break;
	case VK_SUBTRACT:
		m_glRenderer.m_dist += 5;
		break;
	case 'L':
		m_glRenderer.m_light = !m_glRenderer.m_light;
		break;
	case 'C':
		m_glRenderer.m_copterMode = !m_glRenderer.m_copterMode;
		break;
	case 'W':
		m_glRenderer.c_rotation *= 1.6;
		m_glRenderer.c_height += 0.2;
		break;
	case 'S':
		m_glRenderer.c_rotation /= 1.6;
		m_glRenderer.c_height -= 0.2;
